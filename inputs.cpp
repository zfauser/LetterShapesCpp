...
Name: Zach Fauser
Date: March 1st, 2024
Purpose: The purpose of this file is to define the functions that will be used
to handle the user's inputs
*/
...
/*
  Returns:
    outputNumber (int): Returns the inputNumber (string) variable as an integer,
  after it have been error trapped
*/
{
...
/*
  Returns:
    outputLetter (char): Returns the inputLetter (string) variable as a
  character, after it have been error trapped
*/
{
  ...
  while ((cin.fail()) || (inputLetter.length() != 1)) {
    ...
    // If the input is not a single character, ask the user to only enter one
    // character
...
/*
  Returns:
    outputShape (int): Returns inputShape (string) variable as an integer after
  it has been error trapped
*/
{
  string inputShape = "0";
  ...
  cin >> inputShape;
  // While the input is invalid, ask the user to input a number again
  while (cin.fail() || invalidCharacterFound(inputShape) ||
         stoi(inputShape) < 1 || stoi(inputShape) > 4) {
    cin.clear();
    cin.ignore(numeric_limits<streamsize>::max(), '\n');
    cout << BOLD << RED
